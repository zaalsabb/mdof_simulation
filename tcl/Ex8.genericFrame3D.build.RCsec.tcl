...
set filename "$dataDir/nodes.txt";
set fileId [open $filename "w"];

# define NODAL COORDINATES
set MaxNodeID 0;
set Dlevel 100;	# numbering increment for new-level nodes
set Dframe 10000;	# numbering increment for new-frame nodes
for {set frame 1} {$frame <=[expr $NFrame]} {incr frame 1} {
	...
	for {set level 1} {$level <=[expr $NStory+1]} {incr level 1} {
		...
		for {set pier 1} {$pier <= [expr $NBay+1]} {incr pier 1} {
			...
			puts $fileId [format "%i %f %f %f" $nodeID $X $Y $Z];
...
for {set level 2} {$level <=[expr $NStory+1]} {incr level 1} {
	...
	set MasterNodeID [expr 9900000+$level]
...
if {$SectionType == "Elastic"} {
	...
	BuildRCrectSection $ColSecTagFiber $HCol $BCol $cover $cover $IDconcCore  $IDconcCover $IDSteel $numBarsTopCol $barAreaTopCol $numBarsBotCol $barAreaBotCol $numBarsIntCol $barAreaIntCol  $nfCoreY $nfCoreZ $nfCoverY $nfCoverZ $GJ
	BuildRCrectSection $BeamSecTagFiber $HBeam $BBeam $cover $cover $IDconcCore  $IDconcCover $IDSteel $numBarsTopBeam $barAreaTopBeam $numBarsBotBeam $barAreaBotBeam $numBarsIntBeam $barAreaIntBeam  $nfCoreY $nfCoreZ $nfCoverY $nfCoverZ $GJ
	BuildRCrectSection $GirdSecTagFiber $HGird $BGird $cover $cover $IDconcCore  $IDconcCover $IDSteel $numBarsTopGird $barAreaTopGird $numBarsBotGird $barAreaBotGird $numBarsIntGird $barAreaIntGird  $nfCoreY $nfCoreZ $nfCoverY $nfCoverZ $GJ
...
set filename "$dataDir/elements.txt";
set fileId [open $filename "w"];
...
for {set frame 1} {$frame <=[expr $NFrame]} {incr frame 1} {
	for {set level 1} {$level <=$NStory} {incr level 1} {
		for {set pier 1} {$pier <= [expr $NBay+1]} {incr pier 1} {
			...
			puts $fileId [format "%i %i %i" $elemID $nodeI $nodeJ];
		}
	}
}
